...
if(!isset($_GET['section']) and !isset($_GET['period'])){
	...
	$period_id = $_GET['period'];
		...
		if( $section_id == 4 ){
			header("Location: rating_sheet_3.php?section=$section_id&period=$period_id");
		}
}
...
<body class="dt-example dt-example-bootstrap">
	...
	<table class="table table-bordered table-striped">
		<tr><th>STUDENT NAME</th>
			...
			?>
			<th>MAPEH</th><th>GA</th><th>Comments</th>
		</tr>
		<?php
			...
			while($row = $result->fetch_assoc()) {
				...
				echo "<tr><td><a href='report_card.php?id=".$student_id."'>".strtoupper($row['lastname'].", ".$row['firstname']). "</a></td>";
				...
				$ig = number_format(( $non_mapeh + $mapeh ) / ( $non_mapeh_count + 1 ),3); //changed from 2 to 3 decimal places
				//$qg_sql = "SELECT * FROM grade_scale where scale <= $ig LIMIT 1";
				//$qg_result = $conn->query($qg_sql);
				//$qg_row = $qg_result->fetch_assoc();
				//$qg = $qg_row['grade'];
				echo "<td>$mapeh</td><td>$ig</td>";
				//get comments
				if($period_id==1){
