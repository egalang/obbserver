...
<body class="dt-example dt-example-bootstrap">
	...
	<table class="table table-bordered table-striped">
		<tr><th>STUDENT NAME</th>
			...
			?>
			<th>GA</th><th>COMMENTS</th><!--<th>QG</th>-->
		</tr>
		<?php
			...
			if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				$mapeh = 0; $non_mapeh = 0; $non_mapeh_count = 0;
				$student_id = $row['student_id'];
				echo "<tr><td><a href='report_card_ps.php?id=".$student_id."'>".strtoupper($row['lastname'].", ".$row['firstname']). "</a></td>";
				...
				$ig = number_format(( $non_mapeh + $mapeh ) / ( $non_mapeh_count),3);
				...
				//get comments
				if($period_id==1){
					$cmt_sql = "SELECT * FROM character_building WHERE student_id = $student_id AND value_id = 126";
				} elseif ($period_id==2){
					$cmt_sql = "SELECT * FROM character_building WHERE student_id = $student_id AND value_id = 127";
				} elseif ($period_id==3){
					$cmt_sql = "SELECT * FROM character_building WHERE student_id = $student_id AND value_id = 128";
				} else {
					$cmt_sql = "SELECT * FROM character_building WHERE student_id = $student_id AND value_id = 129";
				}
