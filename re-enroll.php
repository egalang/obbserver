<?php
  ...
  $old_enrollee="select * from enrollment_list where old_id=$enrollee_id";
  $enrollee_result = $conn->query($enrollee);
  $old_enrollee_result = $conn->query($old_enrollee);
  $enrollee_row = $enrollee_result->fetch_assoc();

  //check if already enrolled
  if(($enrollee_row['sy']=='2024-2025')or($old_enrollee_result->num_rows>0)){
    header("Location: files_list.php?message=2");
    exit;
  }
  //set new grade level
  $old_grade_level = $enrollee_row['level'];
  $grade_level = "SELECT * FROM grade_levels WHERE id = $old_grade_level";
  $grade_level_result = $conn->query($grade_level);
  $grade_level_row = $grade_level_result->fetch_assoc();
  $new_grade_level_id = $grade_level_row['next_id'];
?>
</script>
</head>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="bcno" name="psa" required placeholder="PSA Birth Certificate Number" value="<?php echo $enrollee_row['psa']; ?>">
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
                ...
                <select class="form-control" id="level" name="level" required>
                        ...
                        if ($levels_result->num_rows > 0) {
                                ...
                                while($levels_row = $levels_result->fetch_assoc()) {
                                        ...
                                        //if ($new_grade_level_id==$levels_row['id']) {
